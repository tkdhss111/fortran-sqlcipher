! SQLCipher Fortran Interfaces 
!
! @author Hisashi Takeda, Ph.D.
! @date 2023-10-19
! @breif
! This source code is a modified version of the following
! SQLite3 interfaces by Philipp Engel.
!
! SQLite3 interfaces:
!
! sqlite3.f90
...
module sqlcipher_interfaces
    ...
    public :: sqlite3_key    ! tkd added
    public :: sqlite3_key_v2 ! tkd added
    ...
    interface
        ! tkd moved sqlite3_util.f90 here
       ...
       ! sqlite3_int64 sqlite3_column_int64(sqlite3_stmt *stmt, int icol)
        ...
        ! tkd added
        function sqlite3_key(db, pkey, nkey) bind(c, name='sqlite3_key')
            import :: c_char, c_int, c_ptr
            implicit none
            type(c_ptr),            intent(in), value :: db
            character(kind=c_char), intent(in)        :: pkey
            integer(kind=c_int),    intent(in), value :: nkey
            integer(kind=c_int)                       :: sqlite3_key
        end function sqlite3_key

        ! tkd added
        function sqlite3_key_v2(db, zdbname, pkey, nkey) bind(c, name='sqlite3_key_v2')
            import :: c_char, c_int, c_ptr
            implicit none
            type(c_ptr),            intent(in), value :: db
            character(kind=c_char), intent(in)        :: zdbname
            character(kind=c_char), intent(in)        :: pkey
            integer(kind=c_int),    intent(in), value :: nkey
            integer(kind=c_int)                       :: sqlite3_key_v2
        end function sqlite3_key_v2
...
contains
    ...
    ! tkd moved from sqlite3_util.f90
...
end module sqlcipher_interfaces
